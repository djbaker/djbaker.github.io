...
_.each = function(collection, action) {
    if (Array.isArray(collection)) {
        ...
        for (var keys in collection) {
            action(collection[keys], keys, collection);
        }
    }
}
...
//     var accepted = _.filter(array, action);
//     var rejected = [];
//     _.each(array, function(value, i, action) {
...
_.map = function(collect, action) {
    var arr = [];
    _.each(collect, function(value, pos, collect) {
        arr.push(action(value, pos, collect))
    })
    return arr;
}
...
_.pluck = function(array, property) {
    return _.map(array, function(value, i, collect) {
...
_.every = function(coll, action) {
    if (_.typeOf(action) === 'function') {
        if ((_.reject(coll, action)).length > 0) {
            return false;
        }
        return true;
    } else {
        var io = true;
        _.each(coll, function(value, i, array) {
            if (!value) {io = false;}
        })
        return io;
    }
...
_.some = function(coll, action) {
    if (_.typeOf(action) === 'function') {
        if ((_.filter(coll, action)).length > 0) {
            return true;
        } return false;
    } else {
        var io = false;
...
_.extend = function(object1, object2) {
    _.each(arguments, function(obj, i, args) {
